...
var es = childProcess.execSync;
var http = require('http');
var _u = require('lodash');

var _aws = '52.193.102.28'
var aws_ip = 'http://'+_aws+':8050';

var _l_f = []; // look out for
// check look out by _u.intersection(_l_f, res)
// push as _l_f.push(elem)
// rem as _l_f.splice(_l_f.indexOf(elem),1)
...
left_m = MotorShield_lib.AdafruitMS1438.MOTOR_M1;                                                                                                                                                                                             
right_m = MotorShield_lib.AdafruitMS1438.MOTOR_M3;                                                                                                                                                                                            
front_m = MotorShield_lib.AdafruitMS1438.MOTOR_M4;

MotorDirCW = MotorShield_lib.AdafruitMS1438.DIR_CW;
MotorDirCCW = MotorShield_lib.AdafruitMS1438.DIR_CCW;
...
Cylon.robot({
name: 'nexus',
events:['_red_alert','_show_image', '_left','left','_right','right','_start','_stop','_forward','forward','_vel', '_log'],
...
devices: {
	f_sensor: {driver:'maxbotix', pin: 2},
	l_sensor: {driver:'maxbotix', pin: 0},
	r_sensor: {driver:'maxbotix', pin: 1}
},
...
identify_objects: function(){                                                                             
	...
	http.get(aws_ip+'/home/leader', function(r){
		...
		r.on('end', function(){
			...
			console.log(body);
			// get keys of JSON now
                        var keys = _u.keys(body);
                        // find	intersection now
                        var p =	_u.intersection(_l_f, keys);
                        if (p.length>0)
                        {
                                console.log('Red alert ! --> ', p);      
				_nexus.emit('_red_alert',p);
                        }
		})
	});
},
